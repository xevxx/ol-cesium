/* eslint-disable @typescript-eslint/no-unused-vars, no-empty, no-use-before-define, no-multi-assign, no-eq-null */
...
export default class HeatmapSynchronizer extends AbstractSynchronizer<any> {
  ...
  destroyCesiumObject(_obj: any) {
    this.cleanupOverlay();
  }
  removeSingleCesiumObject(_c: any, _destroy: boolean) {
    this.cleanupOverlay();
  }
  removeAllCesiumObjects(_destroy: boolean) {
    this.cleanupOverlay();
  }
  ...
  createSingleLayerCounterparts(olLayerWithParents: LayerWithParents) {
    ...
    const captureAnchors = () => {
      ...
      const cart = scene.camera.pickEllipsoid(
        new Cesium.Cartesian2(cxCss, cyCss),
    ...
    const applyFollowTransform = () => {
      if (!this.overlayCanvas || !this.anchorLonLat || !this.anchorScreenC) {
        return;
      }
      ...
      const B = [ex0.x, ey0.x, ex0.y, ey0.y] as [
        number,
        number,
        number,
        number,
      ...
      const T = [ex1.x, ey1.x, ex1.y, ey1.y] as [
        number,
        number,
        number,
        number,
    ...
    const renderHeatmapOverlay = () => {
      ...
      const alphaScale = alphaFromRadiusBlur(radius, blur, {
        r0: radius,
        b0: blur,
        alpha0: 0.6,
      });
      ...
      const ctxCssW = cvs.clientWidth,
        ctxCssH = cvs.clientHeight;
      this.overlayCanvas!.style.transform = ''; // reset baseline
      ctx.clearRect(0, 0, ctxCssW, ctxCssH);
      ctx.globalAlpha = Math.max(
        0,
        Math.min(1, Number(layer.getOpacity?.() ?? 1)),
      );
    ...
    [layer, ...olLayerWithParents.parents].forEach((l: any) => {
      keys.push(
        l.on('change:visible', () => {
          if (this.overlayCanvas) {
            const allVisible = [layer, ...olLayerWithParents.parents].every(
              (p: any) => !!p.getVisible?.(),
            );
    ...
    if (cam?.moveStart) {
      const onStart = () => {
        ...
        if (!this.postRenderUnlisten) {
          ...
          this.postRenderUnlisten = () => {
            try {
              scene.postRender.removeEventListener(fn);
            } catch {}
          };
        }
      };
      cam.moveStart.addEventListener(onStart);
      this.cesiumMoveStartUnlisten = () => {
        try {
          cam.moveStart.removeEventListener(onStart);
        } catch {}
      };
    }
    ...
    if (cam?.moveEnd) {
      ...
      this.cesiumMoveEndUnlisten = () => {
        try {
          cam.moveEnd.removeEventListener(onEnd);
        } catch {}
      };
    } else {
  ...
  private cleanupOverlay() {
    ...
    if (this.cesiumMoveEndUnlisten) {
      this.cesiumMoveEndUnlisten();
      this.cesiumMoveEndUnlisten = null;
    }
    if (this.cesiumMoveStartUnlisten) {
      this.cesiumMoveStartUnlisten();
      this.cesiumMoveStartUnlisten = null;
    }
    if (this.overlayCanvas) {
      this.overlayCanvas.style.transform = '';
    }
