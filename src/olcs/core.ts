...
export function sourceToImageryProvider(
  ...
  | olcsCoreOLImageryProvider
  | MVTImageryProvider
  | MVTWMSImageryProvider
  | SingleTileImageryProvider {
  const shadowSource = source && source.get && source.get('olcs_shadowSource');
  if (shadowSource) {
    const shadowProvider = sourceToImageryProvider(
      olMap,
      shadowSource,
      viewProj,
      olLayer,
  ...
  if (source instanceof olSourceTileImage) {
    ...
    if (skip === false) {
      ...
      if (isTms) {
        ...
        provider = new MVTWMSImageryProvider(olMap, source, viewProj, {
          rectangle,
          styleFunction,
          minimumLevel,
        });
