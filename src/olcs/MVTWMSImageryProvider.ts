...
import type olSourceVectorTile from 'ol/source/VectorTile.js';
...
export default class MVTWMSImageryProvider implements ImageryProvider /* should not extend Cesium.ImageryProvider */ {
  ...
  constructor(
    olMap: Map,
    source: olSourceVectorTile,
    opt_fallbackProj: Projection,
    options: MVTWMSOptions,
  ) {
    this.source_ = source;
      ...
      if (!promise) {
        ...
        if (tileUrlFunction && this.projection_) {
          ...
          if (url) {
            ...
            promise = this.getTileFeatures(url, level, x, y).then(
              (features) => {
